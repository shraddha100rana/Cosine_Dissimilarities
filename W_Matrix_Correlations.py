...
dist = dist.rename(columns = {"level_0": "Destination ID", "level_1": "Origin ID", "Destination Cluster ID": "Miles"})
table = pd.pivot_table(dist, values = "Miles", index = ["Origin ID"], columns = ["Destination ID"], fill_value = 0)
D = table.to_numpy()
...
vol = vol.reset_index()
vol = vol.rename(columns = {"level_0": "Destination ID", "level_1": "Origin ID", "Destination Cluster ID": "Volume"})
table2 = pd.pivot_table(vol, values = "Volume", index = ["Origin ID"], columns = ["Destination ID"], fill_value = 0)
V = table2.to_numpy()
Vt = V.transpose()
